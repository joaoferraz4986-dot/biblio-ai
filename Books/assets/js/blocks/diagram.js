...
(function () {
  ...
  function figure(cls, b, caption, canvas, extra) {
    ...
    if (style)
      Object.keys(style).forEach(function (k) {
        fig.style.setProperty(k, style[k]);
      });
    return fig;
  }
  function captionEl(b, badge) {
    ...
    return h(
      "figcaption",
      { class: "b-diagram__cap" },
      b.caption ? Books.inline.render(b.caption) : null,
      badge,
    );
  }
  function mermaidCanvas(source) {
    return h(
      "div",
      {
        class: "b-diagram__canvas",
        dataset: { engine: "mermaid", source: String(source || "") },
      },
  ...
  function svgCanvas(markup, alt) {
    ...
    var canvas = h("div", {
      class: "b-diagram__canvas b-diagram__canvas--svg svg-frame",
    });
    if (!svg) {
      canvas.appendChild(
        h("p", { class: "b-diagram__error" }, "SVG inválido ou vazio."),
      );
      return canvas;
    }
    if (alt && !svg.getAttribute("aria-label") && !svg.querySelector("title"))
  ...
  B.register({
    ...
    defaults: function () {
      return { caption: "", code: 'flowchart LR\n  A["Início"] --> B["Fim"]' };
    },
    fields: [
      {
        key: "code",
    ...
    summary: function (b) {
      return (
        (b.caption ? Books.inline.toPlain(b.caption) + " · " : "") +
        String(b.code || "").split("\n")[0]
      );
    },
    render: function (b) {
  ...
  B.register({
    ...
    example: {
      ...
      engine: "mermaid",
      prompt:
        "Mostre como um std::shared_ptr compartilha o bloco de controle entre cópias.",
    ...
    fields: [
      ...
      {
        key: "prompt",
        label: "Prompt (o que a IA deve desenhar)",
        kind: "text",
        rows: 3,
      },
      {
        key: "source",
        label: "Resultado da IA (código)",
        kind: "code",
        rows: 9,
        required: true,
      },
    ...
    render: function (b) {
      var badge = h(
        "span",
        {
          class: "b-diagram__badge",
          title: "Diagrama gerado com auxílio de IA",
        },
        Books.icons.get("sparkles", 12),
        "IA",
      );
      var canvas =
        b.engine === "svg"
          ? svgCanvas(b.source, b.caption)
          : mermaidCanvas(b.source);
      var details = b.prompt
        ? h(
            "details",
            { class: "b-diagram__prompt" },
            h("summary", null, "prompt usado"),
            h("p", null, b.prompt),
          )
        : null;
      return figure(
        ...
        b,
        captionEl(b, badge) ||
          h("figcaption", { class: "b-diagram__cap" }, badge),
        canvas,
        details,
      );
    },
  ...
  B.register({
    ...
    defaults: function () {
      return {
        alt: "",
        caption: "",
        svg: '<svg viewBox="0 0 300 80"><rect x="10" y="10" width="280" height="60" rx="8" fill="#243b53" stroke="#76a9fa"/><text x="150" y="46" fill="#f1f5f9" font-size="16" text-anchor="middle">SVG</text></svg>',
      };
    },
    fields: [
      {
    ...
    summary: function (b) {
      return b.alt || b.caption || "SVG";
    },
    render: function (b) {
      var fig = h(
        "figure",
        { class: "b-diagram b-diagram--svg" },
        svgCanvas(b.svg, b.alt),
      );
      if (b.caption)
        fig.appendChild(
          h(
            "figcaption",
            { class: "b-diagram__cap b-diagram__cap--below" },
            Books.inline.render(b.caption),
          ),
        );
      return fig;
    },
  ...
  B.register({
    ...
    render: function (b, ctx) {
      ...
      if (!url)
        fig.appendChild(
          h("div", { class: "b-image__empty" }, "imagem não definida"),
        );
      else {
        var image = h("img", { src: url, alt: b.alt || "", loading: "lazy" });
        image.addEventListener("error", function () {
          image.hidden = true;
          fig.appendChild(
            h(
              "div",
              { class: "b-image__empty" },
              "imagem indisponível — verifique o arquivo: " + b.src,
            ),
          );
        });
